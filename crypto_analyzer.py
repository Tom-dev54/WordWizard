...
def generate_signal(df):
    ...
    if len(clean) < 2:
        return {"signal": "数据不足", "color": "#888888", "direction": "观望",
                "win_rate": "N/A", "confidence": 0, "price": 0, "rsi": 50,
                "macd_hist": 0, "atr_pct": 0, "ema20": 0, "ema50": 0,
                "bb_upper": 0, "bb_lower": 0, "bb_mid": 0,
                "entry": 0, "stop_loss": 0, "take_profit": 0,
                "order_type": "观望", "limit_price": None,
                "bull_score": 0, "bear_score": 0, "is_long": True}
    ...
    return {
        ...
        "bull_score": bull_score, "bear_score": bear_score, "is_long": is_long,
    }
...
def calc_position(capital_u, sig, leverage):
    open_u   = min(capital_u * 0.3, capital_u * STOP_LOSS_PCT / STOP_LOSS_PCT)
...
def fetch_sentiment(symbol):
    ...
    fg_val = fng["value"]
    fg_label = "未知"
    for rng, lbl in {range(0,25):"极度恐惧 😱",range(25,47):"恐惧 😰",
                     range(47,54):"中性 😐",range(54,75):"贪婪 😏",
                     range(75,101):"极度贪婪 🤑"}.items():
        if fg_val in rng:
            fg_label = lbl; break

    sent_bull = sent_bear = 0
    ...
    if fr is not None:
        fr_pct = fr * 100
        if fr_pct > 0.1:
            fr_label = f"+{fr_pct:.4f}%（多头过热，注意轧空风险）"; sent_bear += 1
        elif fr_pct < -0.05:
            fr_label = f"{fr_pct:.4f}%（空头过热，做多有优势）"; sent_bull += 1
        else:
            fr_label = f"{fr_pct:.4f}%（中性）"

    lsr_label = "N/A"
    if lsr is not None:
        if lsr > 1.2:   lsr_label = f"{lsr:.2f}（多头占优）"; sent_bull += 1
        elif lsr < 0.8: lsr_label = f"{lsr:.2f}（空头占优）"; sent_bear += 1
        else:           lsr_label = f"{lsr:.2f}（多空均衡）"
    ...
    return {"fg_val": fg_val, "fg_label": fg_label,
            "fr_label": fr_label, "lsr_label": lsr_label,
            "overall": sent_overall, "bull": sent_bull, "bear": sent_bear}

# ─── AI Analysis ──────────────────────────────────────────────────────────────

def ai_analyze(symbol, sig, sentiment, capital_u, strategy):
    if _ai_client is None:
        return "（AI 分析未启用：请在 Replit Secrets 添加 DEEPSEEK_API_KEY）"
...
def scan_market(strategy="快进快出"):
    ...
    def to_row(t):
        p = float(t["lastPrice"]); hi = float(t["highPrice"]); lo = float(t["lowPrice"])
        return {"symbol": t["symbol"],"price": p,
...
def select_best_coin(capital_u, strategy):
    market = scan_market(strategy)
    coins  = market["快进快出榜"] if strategy == "快进快出" else \
             market["涨幅榜"]   if strategy == "趋势交易" else market["热门榜"]
    reason = "高波动×高成交量" if strategy == "快进快出" else \
             "强势上涨趋势"   if strategy == "趋势交易" else "成交量最大"
    if not coins: return "暂无推荐（数据获取失败）"
    top = coins[0]
    note = ""
    if capital_u < 200 and "BTC" in top["symbol"]:
        alts = [c for c in coins if "BTC" not in c["symbol"]]
        if alts: top = alts[0]; note = "（小资金优先山寨币）"
...
def build_chart(df, symbol):
    try:
        df_plot = df.dropna(subset=["rsi","macd"]).tail(80).copy().reset_index(drop=True)
        n = len(df_plot)
        if n < 5: return None
        ...
        for i, row in df_plot.iterrows():
            c = "#00c851" if row["close"] >= row["open"] else "#ff4444"
            bot = min(row["open"], row["close"])
            h   = abs(row["close"] - row["open"]) or row["close"] * 0.001
            ax1.add_patch(Rectangle((i-0.35, bot), 0.7, h, color=c, zorder=3))
            ax1.vlines(i, row["low"], row["high"], color=c, linewidth=0.8, zorder=2)
        ...
        ax1.fill_between(xs, df_plot["bb_upper"], df_plot["bb_lower"], alpha=0.05, color="#4a90d9")
        ...
        ax1.set_title(f"{symbol}  |  Last: ${df_plot['close'].iloc[-1]:,.4f}", color="white", fontsize=10, pad=8)
        ax1.legend(loc="upper left", fontsize=6, framealpha=0.3, labelcolor="white", facecolor="#1a1a2e")
        ax1.grid(axis="y", color="#1f1f2e", lw=0.5)
        for i, row in df_plot.iterrows():
            c = "#00c851" if row["close"] >= row["open"] else "#ff4444"
            ax2.bar(i, row["volume"], color=c, alpha=0.7, width=0.7)
        ...
        ax3.fill_between(xs, df_plot["rsi"], 70, where=df_plot["rsi"]>=70, alpha=0.2, color="#ff4444")
        ax3.fill_between(xs, df_plot["rsi"], 30, where=df_plot["rsi"]<=30, alpha=0.2, color="#00c851")
        ax3.set_ylim(0, 100)
        ...
        ax3.set_xticks(ticks); ax3.set_xticklabels(labels, rotation=30, ha="right", fontsize=6, color="#aaaaaa")
...
def card(title, body, border="#333"):
    return (f'<div style="background:#161b22;border:1px solid {border};'
...
def build_price_html(ticker, sig):
    ...
    cc  = "#00c851" if chg >= 0 else "#ff4444"
    body = (f'<div style="font-size:26px;font-weight:bold;color:white">{_fmt(p)}</div>'
            f'<div style="font-size:16px;color:{cc};margin-top:4px">{"▲" if chg>=0 else "▼"} {chg:+.2f}%</div>'
            f'<div style="font-size:12px;color:#888;margin-top:4px">高: {_fmt(hi)} &nbsp; 低: {_fmt(lo)}</div>')
    return card("当前价格", body)

def build_signal_html(sig):
    body = (f'<div style="font-size:24px;font-weight:bold;color:{sig["color"]}">{sig["signal"]}</div>'
            f'<div style="font-size:13px;color:#ccc;margin-top:6px">{sig["direction"]} &nbsp;|&nbsp; 胜率 {sig["win_rate"]}</div>'
            f'<div style="font-size:12px;color:#888;margin-top:4px">置信度 {sig["confidence"]}%</div>')
    return card("交易信号", body, sig["color"])

def build_rec_html(sig, pos, leverage):
    hint = sig["order_type"] + (f' @ {_fmt(sig["limit_price"])}' if sig["limit_price"] else "")
    tp_c, sl_c = "#00c851", "#ff4444"
    body = (f'<table style="width:100%;border-collapse:collapse;font-size:13px;color:#ddd">'
            f'<tr><td style="padding:4px 0;color:#888">📌 入场方式</td><td style="font-weight:bold">{hint}</td></tr>'
            f'<tr><td style="padding:4px 0;color:#888">💰 建议仓位</td><td><b>{pos["open_u"]}U</b>（{leverage}倍杠杆 = {pos["nominal_u"]:.0f}U）</td></tr>'
            f'<tr><td style="padding:4px 0;color:{sl_c}">🛡 止损价</td><td><b>{_fmt(sig["stop_loss"])}</b>（预计亏损 -{pos["loss_u"]}U）</td></tr>'
            f'<tr><td style="padding:4px 0;color:{tp_c}">🎯 止盈价</td><td><b>{_fmt(sig["take_profit"])}</b>（预计盈利 +{pos["profit_u"]}U）</td></tr>'
            f'<tr><td style="padding:4px 0;color:#888">⚡ 杠杆建议</td><td><b>{leverage}倍</b>（新手建议 ≤ 3倍）</td></tr>'
            f'</table>')
    return card("操作建议", body, "#4a90d9")

def build_indicators_html(sig, sentiment):
    rsi = sig["rsi"]
    rc = "#ff4444" if rsi>70 else ("#00c851" if rsi<30 else "#ffab00")
    rl = "超买" if rsi>70 else ("超卖" if rsi<30 else "中性")
    mc = "#00c851" if sig["macd_hist"]>0 else "#ff4444"
    ml = "多头" if sig["macd_hist"]>0 else "空头"
    ec = "#00c851" if sig["ema20"]>sig["ema50"] else "#ff4444"
    el = "多头排列" if sig["ema20"]>sig["ema50"] else "空头排列"
    body = (f'<div style="display:flex;flex-wrap:wrap;gap:8px">'
            ...
            f'<div style="font-size:18px;font-weight:bold;color:{rc}">{rsi:.1f}</div>'
            f'<div style="font-size:11px;color:{rc}">{rl}</div></div>'
            f'<div style="flex:1;min-width:120px;background:#1a1a2e;border-radius:8px;padding:10px">'
            f'<div style="font-size:11px;color:#888">MACD柱</div>'
            f'<div style="font-size:18px;font-weight:bold;color:{mc}">{sig["macd_hist"]:+.5f}</div>'
            f'<div style="font-size:11px;color:{mc}">{ml}</div></div>'
            f'<div style="flex:1;min-width:120px;background:#1a1a2e;border-radius:8px;padding:10px">'
            f'<div style="font-size:11px;color:#888">均线趋势</div>'
            f'<div style="font-size:14px;font-weight:bold;color:{ec}">{el}</div>'
            ...
            f'</div>')
    return card("技术指标 & 市场情绪", body)

def build_sentiment_html(sentiment):
    fg = sentiment["fg_val"]
    bc = "#00c851" if fg<40 else ("#ff4444" if fg>60 else "#ffab00")
    body = (f'<div style="margin-bottom:8px;font-size:13px;color:#ccc"><b>恐惧贪婪指数：</b> '
            f'<span style="color:{bc};font-size:18px;font-weight:bold">{fg}</span> / 100 — {sentiment["fg_label"]}</div>'
            f'<div style="background:#333;border-radius:4px;height:8px;margin-bottom:10px">'
            f'<div style="background:{bc};width:{fg}%;height:100%;border-radius:4px"></div></div>'
            f'<div style="font-size:12px;color:#aaa;line-height:1.8">'
            f'资金费率：{sentiment["fr_label"]}<br>多空持仓比：{sentiment["lsr_label"]}<br>'
            f'综合情绪：<b style="color:#ffab00">{sentiment["overall"]}</b></div>')
    return card("市场情绪分析", body)

def build_scan_html(market):
    ts = 'style="width:100%;border-collapse:collapse;font-size:12px"'
    hd = ('<tr style="color:#888;border-bottom:1px solid #333">'
    ...
    def rows(coins):
        if not coins: return "<tr><td colspan='3' style='color:#888;padding:4px'>暂无数据</td></tr>"
        return "".join(f'<tr><td style="padding:3px 6px;color:white">{c["symbol"]}</td>'
                        f'<td style="padding:3px 6px;color:{"#00c851" if c["change"]>=0 else "#ff4444"}">{c["change"]:+.2f}%</td>'
                        f'<td style="padding:3px 6px;color:#aaa">${c["volume"]/1e6:.0f}M</td></tr>' for c in coins)
    def sec(title, coins):
        return (f'<div style="flex:1;min-width:180px"><div style="font-size:12px;color:#888;margin-bottom:6px">{title}</div>'
                f'<table {ts}>{hd}{rows(coins)}</table></div>')
    body = (f'<div style="display:flex;flex-wrap:wrap;gap:12px">'
            f'{sec("🔥 热门榜",market["热门榜"])}{sec("📈 涨幅榜",market["涨幅榜"])}'
            f'{sec("📉 跌幅榜",market["跌幅榜"])}{sec("⚡ 快进快出",market["快进快出榜"])}</div>')
    return card("市场扫描", body)

def build_ai_html(ai_text):
    return card("🤖 AI 综合分析（DeepSeek）",
                f'<div style="font-size:13px;color:#ddd;line-height:1.8;white-space:pre-wrap">{ai_text}</div>',
                "#7c3aed")

# ─── Main Orchestrator ────────────────────────────────────────────────────────

def run_analysis(symbol, timeframe_label, capital_u, strategy):
    ...
    if df is None or len(df) < 60:
        ...
        return err, err, err, err, err, None, err, err, timestamp
    ...
    return (build_price_html(ticker, sig), build_signal_html(sig),
            build_rec_html(sig, pos, leverage), build_indicators_html(sig, sentiment),
            build_sentiment_html(sentiment), chart,
            build_scan_html(market), build_ai_html(ai_text), timestamp)
...
杠杆交易可能导致超额亏损，您可能损失全部本金。请只投入您能承受损失的资金。</div>"""

CSS = ".gradio-container{max-width:100%!important}.main{padding:8px!important}footer{display:none!important}"
...
with gr.Blocks(title="加密货币智能交易分析器", css=CSS,
    ...
    with gr.Row():
        dd_symbol   = gr.Dropdown(choices=POPULAR_PAIRS, value="BTCUSDT", label="选择交易对", scale=3)
        dd_tf       = gr.Dropdown(choices=list(TIMEFRAME_MAP.keys()), value="1小时", label="时间周期", scale=2)
        num_capital = gr.Number(value=100, label="本金（USDT）", minimum=10, scale=2)
        dd_strategy = gr.Dropdown(choices=["快进快出","趋势交易"], value="快进快出", label="策略类型", scale=2)
    ...
    try:
        ...
        gr.HTML("<script>setInterval(function(){var b=document.querySelector('button.lg');if(b)b.click();},60000);</script>")

if __name__ == "__main__":
    _share = os.environ.get("GRADIO_SHARE", "true").lower() == "true"
    _port  = int(os.environ.get("GRADIO_SERVER_PORT", "7861"))
    demo.queue().launch(server_name="0.0.0.0", server_port=_port, share=_share, show_error=True)
