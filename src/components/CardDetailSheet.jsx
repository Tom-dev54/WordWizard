...
export default function CardDetailSheet({ card, onClose, onJournal }) {
  ...
  return (
    <Sheet onClose={onClose} contentStyle={{ maxHeight: '95vh', padding: '20px 0 48px' }}>
      {(dismiss) => (
        <>
          <div className="sheet-handle" />

          {/* Card hero */}
          <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', padding: '0 20px', marginBottom: 22 }}>
            <div
          ...
          <div style={{ display: 'flex', gap: 10, padding: '22px 20px 0' }}>
            <button
              onClick={() => { dismiss(); setTimeout(() => onJournal && onJournal(card), 420) }}
              style={{
